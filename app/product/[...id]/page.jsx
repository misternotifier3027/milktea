...
const ProductPage = () => {
    ...
    const addProductToCard = () => {
        ...
        if (orderProduct.quantity != 0) {
            ...
            if (params.id[1]) {
                const newCard = card.filter((item, i) => i != params.id[1])
                updateCard(newCard)
            }
        }
        router.push("/")
            ...
            !loading ? (
                <div tabindex="-1" className={` bg-white overflow-hidden md:max-h-screen transition-opacity duration-300 animate-zoom-in md:p-6`}>
                    <div className="relative w-full ">
                        <button onClick={() => router.push("/")} type="button" className="absolute z-30 bg-bodydark top-2 p-1 left-2 text-white rounded-full w-8 h-8 ml-auto inline-flex justify-center items-center" >
                                    ...
                                    <div className='flex items-center gap-5'>
                                        <div className='flex gap-4 items-center'>
                                            <div className='bg-bodydark rounded-full p-1 text-white cursor-pointer'>
                                                <svg onClick={() => {orderProduct.quantity == 0 ? setOrderProduct({...orderProduct, quantity: 0}) : setOrderProduct({...orderProduct, quantity: orderProduct.quantity -= 1})}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
